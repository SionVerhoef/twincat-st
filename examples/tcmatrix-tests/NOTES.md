...
The harder case is a device or sequence FB, where a test must drive the FB across many cycles before asserting. `templates/FB_ExampleTestSuite.TcPOU` sketches that shape (a `FOR` loop of calls before the assertion), and getting there is why `naming-conventions.md` argues for FBs whose only contact with the outside world is their interface — an FB that reaches into a GVL cannot be tested this way.
